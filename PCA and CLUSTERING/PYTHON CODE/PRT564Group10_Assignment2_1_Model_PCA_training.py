...
def train_naive_bayes_original(df):
    ...
    # Build Naive Bayes classifier on original data
    clf = GaussianNB().fit(X_train, y_train)
    y_train_pred = clf.predict(X_train)
    print("\nPrediction accuracy for the training dataset:")
    print("{:.2%}".format(metrics.accuracy_score(y_train, y_train_pred)))
...
def train_naive_bayes_standardized(df):
    ...
    y_train_pred_std = clf_std.predict(X_train_std)
...
def train_naive_bayes_pca(df, num_components_1,num_components_2):
    ...
    pca2 = PCA(n_components=num_components_2)
    # Apply PCA to original data and standardized data
    X_train_pca = pca1.fit_transform(X_train)
    X_train_std_pca = pca2.fit_transform(X_train_std)
    ...
    print("\nPrediction accuracy for the training dataset:")
    print("{:.2%}".format(metrics.accuracy_score(y_train, y_train_pred_pca)))
    

    # Build Naive Bayes classifier on standardized data after PCA
    clf_std_pca = GaussianNB().fit(X_train_std_pca, y_train)
    y_train_pred_std_pca = clf_std_pca.predict(X_train_std_pca)

    
    print("\nPrediction accuracy for the training dataset (with standardisation):")
    print("{:.2%}".format(metrics.accuracy_score(y_train, y_train_pred_std_pca)))
    return 
...
print('Applying PCA to training datasets')
